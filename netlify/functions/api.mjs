...
async function handleAi(event) {
  ...
  if (!text) return json(502, { error: describeEmptyAiResponse(data) });
...
function signState(payload) {
  ...
  return createHmac("sha256", secret).update(payload).digest("hex");import { getStore } from "@netlify/blobs";
import { createHmac, randomBytes, timingSafeEqual } from "node:crypto";
